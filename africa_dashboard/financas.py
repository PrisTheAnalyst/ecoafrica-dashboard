...
import pandas as pd
from components import kpi, insight, section, fig_base, add_vline
from config import (ANGOLA, BLUE, BLUE_L, GREEN, GREEN_L, RED, GOLD,
                    WHITE, MUTED, MUTED_L, BG, CARD, CARD_L, GRID, BORDER)

# Limiares de risco fiscal (convenção FMI/World Bank)
LIMIAR_DIVIDA_ALTO  = 100   # % PIB
LIMIAR_DIVIDA_MED   = 60    # % PIB
LIMIAR_INF_ALTO     = 20    # %
LIMIAR_INF_MED      = 10    # %
...
def render(p, s, r, m):
    ...
    if filtro_risco == "Alto (≥100% PIB)":    pf = pf[pf["Dívida_Pública_%_PIB_2022"] >= LIMIAR_DIVIDA_ALTO]
    elif filtro_risco == "Moderado (60–100%)":pf = pf[(pf["Dívida_Pública_%_PIB_2022"] >= LIMIAR_DIVIDA_MED) & (pf["Dívida_Pública_%_PIB_2022"] < LIMIAR_DIVIDA_ALTO)]
    elif filtro_risco == "Controlado (<60%)": pf = pf[pf["Dívida_Pública_%_PIB_2022"] < LIMIAR_DIVIDA_MED]
    ...
    # ── Gráfico A — Dívida pública ranking ───────────────────────────────────
    ...
    if df_div.empty:
        ...
        insight(
            ...
            f"acima do limiar de risco de 100%. "
        ...
        fig_a = fig_base(height=max(300, len(df_div) * 26))
        fig_a.add_trace(go.Bar(
            x=df_div["Dívida_Pública_%_PIB_2022"], y=df_div["País"], orientation="h",
            marker=dict(color=cores_div, line=dict(width=0)),
            text=df_div["Dívida_Pública_%_PIB_2022"].apply(lambda v: f"{v:.0f}%"),
            textposition="outside", textfont=dict(color=WHITE, size=9, family="JetBrains Mono"),
    ...
    with col_b:
        ...
        df_inf = pf.dropna(subset=["Inflação_2022_%"]).sort_values("Inflação_2022_%", ascending=True)
        if df_inf.empty:
            ...
            fig_b = fig_base(height=max(280, len(df_inf) * 22))
            fig_b.add_trace(go.Bar(
                x=df_inf["Inflação_2022_%"], y=df_inf["País"], orientation="h",
                marker=dict(color=cores_inf, line=dict(width=0)),
            ...
            fig_b.add_vline(x=LIMIAR_INF_MED,  line_dash="dash", line_color=GOLD, line_width=1,
                            annotation_text="10% — Atenção", annotation_font_color=GOLD,
                            annotation_position="top right", annotation_font_size=9)
            fig_b.add_vline(x=LIMIAR_INF_ALTO, line_dash="dash", line_color=RED,  line_width=1,
                            annotation_text="20% — Crítico", annotation_font_color=RED,
                            annotation_position="top right", annotation_font_size=9)
    ...
    if df_risco.empty:
        ...
        # Classificação de risco combinado
    ...
    def estilo_val(df):
        ...
        for i, row in df.iterrows():
            ...
            inf = row["Inflação (%)"]
            if not _pd.isna(d):
                if d >= LIMIAR_DIVIDA_ALTO:     styles.loc[i, "Dívida (% PIB)"] = f"color:{RED};font-weight:600"
                elif d >= LIMIAR_DIVIDA_MED:    styles.loc[i, "Dívida (% PIB)"] = f"color:{GOLD};font-weight:600"
                else:                           styles.loc[i, "Dívida (% PIB)"] = f"color:{GREEN};font-weight:600"
            if not _pd.isna(inf):
                if inf >= LIMIAR_INF_ALTO:      styles.loc[i, "Inflação (%)"] = f"color:{RED};font-weight:600"
                elif inf >= LIMIAR_INF_MED:     styles.loc[i, "Inflação (%)"] = f"color:{GOLD};font-weight:600"
                else:                           styles.loc[i, "Inflação (%)"] = f"color:{GREEN};font-weight:600"
        return styles
    ...
    styled = (
        ...
        .set_properties(**{"background-color": CARD, "color": WHITE,
